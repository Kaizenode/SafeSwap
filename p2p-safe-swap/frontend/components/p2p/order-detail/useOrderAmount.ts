...
export const MODE_COPY: Record<OrderMode, { amountLabel: string; cta: string }> = {
  buy: { amountLabel: "Amount to pay", cta: "Start trade" },
  sell: { amountLabel: "Amount to sell", cta: "Start trade" },
};
...
export function useOrderAmount({ order, onStartTrade }: UseOrderAmountOptions) {
  ...
  const error = !hasAmount
    ...
    ? `Minimum amount is ${formatAmount(min)} ${order.currencyPair.base}`
    : amount > max
    ? `Maximum amount is ${formatAmount(max)} ${order.currencyPair.base}`
    : null;
