...
export async function POST(request: NextRequest) {
  ...
  const hasValidIndexes =
    Array.isArray(body.milestoneIndexes) &&
    body.milestoneIndexes.length > 0 &&
    body.milestoneIndexes.every((index) => typeof index === "number" && Number.isInteger(index));
  ...
  if (
    ...
    return NextResponse.json(
      { error: "contractId, approver, and milestoneIndexes are required" },
      { status: 400 }
    );
  }
  ...
  try {
    const data = await trustlessWork.escrow.approveMilestones({
      ...
      milestoneIndexes: body.milestoneIndexes as number[],
    });
